"""
Contains class used to represent data
"""
...
class Dataset:
    ...
    Contains functions to obtain labels etc, as well as several types of feature representations.
    """
    ...
    def __init__(self, data: Union[pd.DataFrame, str]) -> None:
        ...
        Assumptions: the dataframe (or the csv) contains at least columns:
            - 'smiles' [str]: SMILE representation of molecule,
            e.g. 'Cn1ccnc1SCC(=O)Nc1ccc(Oc2ccccc2)cc1'
            - 'P1' [int]: whether molecule has property P1: 1 if yes, 0 otherwise
            (if the dataset is used only to make predictions or evaluate a model,
            'P1' is not necessary)
        """
    ...
    def get_features(self, feature_type=FeatureType.MORGAN) -> np.ndarray:
        """
        Return an array of features corresponding the dataset's SMILEs,
        based on desired feature_type.
        If feature_type is:
